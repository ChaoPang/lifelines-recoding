(function($, molgenis) {
	...
	molgenis.createTable = function(options) {
		...
		var addCodeFunction = (options.addCode
				&& typeof options.addCode === 'function' ? options.addCode
				: molgenis.defaultAddCode);
		...
		if (hits.length > 0) {
			...
			$('<tr />')
					.append(
							'<th>Name</th><th>Code</th><th>Code system</th><th>Score</th><th>Frequency</th><th>Select</th>')
					.appendTo(table);
			$.each(hits, function(index, hit) {
				var columnValueMap = hit.columnValueMap;
				var row = $('<tr />').append(
						'<td>' + columnValueMap.name + '</td>').append(
						'<td>' + columnValueMap.code + '</td>').append(
						'<td>' + columnValueMap.codesystem + '</td>').append(
						'<td>' + hit.score + '%</td>').append(
						'<td>' + hit.frequency + '</td>').append(
				...
				$(checkbox).click(
						function() {
							if ($(checkbox).is(':checked')) {
								table.find(
										'input:checkbox:not(:eq(' + index
												+ '))').attr('checked', false);
							}
						});
			});

			var tableContainer = $('<div />').addClass('row-fluid').appendTo(
					parentElement);
			...
			$('<div />').addClass('span12').append(table).appendTo(
					tableContainer);
			$('<div />').addClass('span12').append(selectUnknowButton).append(
					addCodeButton).append(selectCodeButton).appendTo(
					controlContainer);

			selectCodeButton.click(function() {
				$.each(table.find('input:checkbox'), function(index, checkbox) {
					if ($(checkbox).is(':checked')) {
						addCodeFunction(queryString, $(checkbox).parents(
								'tr:first').data('searchHit'), false);
						setTimeout(function() {
							location.reload();
						}, 1500);
					}
			...
			$
					.ajax({
						...
						success : function(data) {
							...
							table
									.append('<tr><th>Name</th><th>Code</th><th>Code system</th><th>Original</th><th>Frequency</th><th>date added</th><th>Delete</th></tr>');
							$
									.each(
											data.results,
											function(index, hit) {
												...
												row
														.append('<td>'
																...
																+ columnValueMap.code
																+ '</td><td>'
																+ columnValueMap.codesystem
																+ '</td><td>'
																+ columnValueMap.original
																+ '</td><td>'
																+ hit.frequency
																+ '</td><td>'
																+ hit.columnValueMap.date
																+ '</td>');
												$('<td />')
														.append(deleteButton)
														.appendTo(row);
															...
															if (!hit.columnValueMap.original) {
																...
																molgenis
																		.createAlert(
																				[ {
																					'message' : 'The code '
																							+ hit.columnValueMap.code
																							+ ' ( <strong>'
																							+ hit.columnValueMap.name
	...
	molgenis.retrieveBackups = function(parentElement) {
		$
				.ajax({
					...
					contentType : 'application/json',
					success : function(data) {
						var table = $('<table />').addClass('table').appendTo(
								parentElement);
						table
								.append('<tr><th>Job name</th><th>Code system</th><th>Date</th><th style="width:20%;">Recover</th></tr>');
						$
								.each(
										data.results,
										function(index, hit) {
											...
											var row = $('<tr />')
													...
													.append(
															'<td>'
																	+ new Date(
																			hit.columnValueMap.addedDate)
																	+ '</td>')
													.appendTo(table);
											$('<td />').append(recoveyButton)
													.appendTo(row);
											if (!data.notAllow) {
												recoveyButton
														.click(function() {
															...
															confirmButton
																	.click(function() {
																		molgenis
																				.recoverBackup(hit.columnValueMap.name);
																	});
															cancelButton
																	.click(function() {
																		recoveyButton
