(function($, molgenis) {
				...
				.ajax({
					...
					success : function(data) {
						if (data.backupExists) {
							var layoutDiv = $('<div />').addClass(
									'offset3 span6 well').appendTo(
									parentElement);
							var textDiv = $('<div />').addClass('row-fluid')
									.appendTo(layoutDiv);
							var buttonDiv = $('<div />').addClass('row-fluid')
									.appendTo(layoutDiv);
							...
							$('<div />')
									.addClass('span12')
									.append(
											'The backup exists, would you like to recover the most recent session?</br></br>')
									.appendTo(textDiv);
							$('<div />').addClass('offset6 span3').append(
									recoveryButton).appendTo(buttonDiv);
							$('<div />').addClass('span3')
									.append(dismissButton).appendTo(buttonDiv);
							...
							recoveryButton
									.click(function() {
										molgenis
												.recoverBackup(data.backup[0].columnValueMap.name);
									});
						}
					}
				});
	...
	molgenis.retrieveResult = function(controlContainer, resultContainer,
			codeSystem, isMapped, click, maxNumber) {
		...
		$.ajax({
			...
			contentType : 'application/json',
			success : function(data) {
				createReportTable(data, controlContainer, resultContainer,
						isMapped);
			}
		});
		function createReportTable(data, parentElement, resultContainer,
				matched) {
			var matchingResults = data.results;
			var maxNumber = data.maxNumber;
			if (matchingResults.length > 0) {
				parentElement.find('.control-button-div').remove();
				var showResultButton = $('<button class="btn '
						+ (matched ? 'btn-primary' : 'btn-info')
						+ '" type="button" style=float:right;>Show result</button>');
				$('<div />').addClass('span4 control-button-div').append(
						showResultButton).appendTo(parentElement);
				showResultButton
						.click(function() {
							if (!matched) {
									...
									.append(
											'<th>Input</th><th>Individuals</th>'
													+ (matched ? '<th>Matched code</th><th>Code system</th><th>Score</th><th>date added</th><th>Delete</th>'
															: '<th style="text-align:center;">Curation</th>'))
									.appendTo(table);
							$
									.each(
											matchingResults,
											function(index, recodeResponse) {
												var row = $('<tr />')
														.append(
																'<td>'
																		+ recodeResponse.queryString
																		+ '</td>')
														.append(
																'<td>'
																		+ Object
																				.keys(recodeResponse.identifiers).length
																		+ '</td>');
												...
												if (matched) {
													...
													row
															.append(
																	'<td>'
																			+ recodeResponse.hit.columnValueMap.code
																			+ ' : '
																			+ recodeResponse.hit.columnValueMap.name
																			+ '</td>')
															.append(
																	'<td>'
																			+ recodeResponse.hit.columnValueMap.codesystem
																			+ '</td>')
															.append(
																	'<td>'
																			+ recodeResponse.hit.score
																			+ '%</td>');
													$('<td />')
															.append(
																	recodeResponse.dateString)
															.appendTo(row);
													$('<td />').append(
															deleteButton)
															.appendTo(row);
																		...
																		.findCode(
																				...
																				function(
																					...
																					var customSearchGroup = $(
																							...
																							.append(
																									clearButton);
																					default_options.parentElement
																							.append('<div class="row-fluid"><legend></legend></div>');
																					var controlDiv = $(
							...
							var selectController = $(
									...
									.css('float', 'right');
							selectController.find('#option-' + maxNumber).attr(
									'selected', true);
							var layoutDiv = $('<div />')
									.addClass('row-fluid')
									.append(
											'<span class="large-text"><strong>'
													+ (matched ? 'Matched results'
															: 'Unmatched results')
													+ '</strong></span>')
									.append(selectController)
									.append('<br><br>').append(table);
							resultContainer
									.empty()
									.append(
											'<div class="row-fluid"><legend></legend></div>')
									.append(layoutDiv);
							selectController.change(function() {
								molgenis.retrieveResult(controlContainer,
										resultContainer, codeSystem, isMapped,
										true, $(this).val());
							});
						});
				if (click)
