...
public class DutchNGramAlgorithm
{
	private static int nGrams = 2;
	private final DutchStemmer stemmer = new DutchStemmer();
	...
	public float stringMatching(String queryOne, String queryTwo)
	{
		return stringMatching(queryOne, queryTwo, true);
	}

	public float stringMatching(String queryOne, String queryTwo, boolean removeStopWords)
	{
		float similarityScore = calculateScore(createNGrams(queryOne.toLowerCase().trim(), removeStopWords),
	 ...
	 * @return
	 */
	private Set<String> createNGrams(String inputQuery, boolean removeStopWords)
	{
	 ...
	 * @return
	 */
	private float calculateScore(Set<String> inputStringTokens, Set<String> ontologyTermTokens)
	{
		int matchedTokens = 0;
